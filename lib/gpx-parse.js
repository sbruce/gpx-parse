    ...
    gpxTracks.forEach(function(currentTrack) {
      ...
      currentTrack.trkseg.forEach(function(currentSegment) {
        ...
        currentSegment.trkpt.forEach(function(trackPoint) {
          ...
          var extensions = null;
          if (trackPoint.hasOwnProperty('extensions')) {
            extensions = parseExtensions(trackPoint.extensions);
          }
  				trackSegement.push(new GpxWaypoint(trackPoint.$.lat, trackPoint.$.lon, elevation, trackPoint.time, null, null, null, null, null, null, null, null, null, extensions));
  			});
...
/**
* Parse the track extensions
**/
var parseExtensions = function(input) {
  var extensions = {};
  for (var i = 0; i < input.length; i++) {
    if (input[i].hasOwnProperty('gpxtpx:TrackPointExtension')) {
      var ext = input[i]['gpxtpx:TrackPointExtension'];
      for (var j = 0; j < ext.length; j++) {
        for (let key of Object.keys(ext[j])) {
          // Strip the gpxtpx: from the extensions
          // Object.prototype.toString.call([]) ==
          var name = key.substr(key.search(':') + 1);
          if (name === '') {
            name = key;
          }
          extensions[name] = ext[j][key];
        }
      }
    }
  }
